...
/**
 * Hook for tracking screen time with idle detection
 */
export function useScreenTimeTracking(screenName: string, idleThresholdSeconds: number = 30) {
  ...
  const recordActivity = useCallback(() => {
    ...
    // If was idle and now active
    if (isIdle.current && idleTime < idleThresholdSeconds) {
      isIdle.current = false;
    }

    // Check if became idle
    if (!isIdle.current && idleTime >= idleThresholdSeconds) {
      isIdle.current = true;
      Analytics.trackIdleDetected(screenName, Math.round(idleTime));
    }

    lastActivityTime.current = now;
  }, [screenName, idleThresholdSeconds]);
 ...
 */
export function useScrollTracking(screenName: string) {
  ...
  return { trackScroll, getScrollPercent };
}
